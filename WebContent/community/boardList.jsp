...
// 끝 행번호 계산 (10, 20, 30, 40..)
 ...
 if(cnt != 0){
	// boardList = bdao.getBoardList();
	 ...
	 boardList = cdao.getBoardList(startRow, pageSize);
 }
    ...
    <tbody>
      <tr>
        ...
        <% 
          ...
          %>
        <a href="content.jsp?bno=<%=cb.getBno()%>&pageNum=<%=pageNum%>"><%=cb.getSubject() %></a>
        ...
        <td></td>
        <td></td>
        <td colspan="3"><%=cb.getName() %></td>  
        <th></th>
        <th></th>
        <th></th>
        <td colspan="2"><%=cb.getDate() %></td>
        <th></th>
        <th></th>
        <td><%=cb.getReadcount() %></td>
        <td><%=cb.getIp() %></td>
      </tr>
