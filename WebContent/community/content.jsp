<%@page import="com.itwillbs.community.CommunityBean"%>
...
cdao.updateReadCount(bno);

CommunityBean cb = cdao.getBoard(bno);
...
<div class="container">
  ...
  <table class="table table-hover">
    ...
    <tbody>
      <tr>
        ...
        <td></td>
        <td><%=cb.getDate() %></td>
        <td colspan="5"><%=cb.getReadcount() %></td>
      </tr>
      <tr>
        <td colspan="5"><p style="white-space:pre;"><%=cb.getContent() %></p></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
  ...
  if(id!=null && id.equals(cb.getName())){
  
  %>
  
  <input type="button" class="btn btn-default" onclick="location.href='updateForm.jsp?bno=<%=cb.getBno() %>&pageNum=<%=pageNum %>'" value="수정">
  
  <input type="button" class="btn btn-default" onclick="location.href='deleteForm.jsp?bno=<%=cb.getBno()%>&pageNum=<%=pageNum%>'" value="삭제">
  <%
      }else if(id==null || !id.equals(cb.getName())){  //로그인 정보가 없거나(아이디가 null) 또는 아이디와 이름이 같지 않은 경우
        	      
          }
              
              %>
  <input type="button" class="btn btn-default" onclick="location.href='reWriteForm.jsp?bno=<%=cb.getBno()%>&re_ref=<%=cb.getRe_ref()%>&re_lev=<%=cb.getRe_lev()%>&re_seq=<%=cb.getRe_seq()%>'" value="답글">
