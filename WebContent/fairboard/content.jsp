<%@page import="com.itwillbs.fair.FairBoardBean"%>
...
adao.updateReadCount(bno);

FairBoardBean ab = adao.getBoard(bno);
...
<div class="container">
  ...
  <table class="table table-hover">
    ...
    <tbody>
      <tr>
        ...
        <td></td>
        <td><%=ab.getDate() %></td>
        <td colspan="3"><%=ab.getReadcount() %></td>
      </tr>
      <tr>
        <td colspan="5"><img src = "../upload/<%=ab.getFile() %>" style="width:500px; height:500px;">
        <p style="white-space:pre;"><%=ab.getContent() %></p> <p></td>
        <td></td>
        <td></td>
      </tr>
      <tr>
      <td>Download</td>
        <th colspan="3"><a href="file_down.jsp?file_name=<%=ab.getFile() %>"><%=ab.getFile() %></a></th>
        <td></td>
        <td></td>
      </tr>
  ...
  if(id!=null && id.equals(ab.getName())){
  
  %>
  
  <input type="button" class="btn btn-default" onclick="location.href='updateForm.jsp?bno=<%=ab.getBno() %>&pageNum=<%=pageNum %>'" value="수정">
  
  <input type="button" class="btn btn-default" onclick="location.href='deleteForm.jsp?bno=<%=ab.getBno()%>&pageNum=<%=pageNum%>'" value="삭제">
  <%
      }else if(id==null || !id.equals(ab.getName())){  //로그인 정보가 없거나(아이디가 null) 또는 아이디와 이름이 같지 않은 경우
